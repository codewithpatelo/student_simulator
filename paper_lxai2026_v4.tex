...
Our central result is a decomposition rather than a verdict: of the gap to a state-estimating baseline, a perfect oracle $g$ buys $7.4\%$, re-tuning six hand-chosen coefficients buys $27.7\%$, and the rest is neither. Giving each signal its own tuning budget --- five independent searches, validated on 50 profiles never used for tuning --- the ordering is monotone in signal quality, but by only $\approx0.05$ against a remaining distance of $0.4$--$0.6$, and a \emph{learned} $g$ is indistinguishable from a constant. Knowing what you need helps; it is nowhere near enough. Where regulation earns a role is viability, and there too we narrow an earlier claim: the homeostatic agent with rest disabled still reaches $93.5\%$ dropout against the estimator's $100\%$, so viability is bought by having \emph{some} rest mechanism.

Four contributions follow: the decomposition, with paired effect sizes and confidence intervals and replicated on the goal metric; the matched-tuning result, which bounds how much the SSP is worth solving, since a \emph{learned} signal does not beat a constant; a methodology --- dose--response sweeps with per-point gating, hashed predictions, multiplicity-corrected existence tests, and multi-seed tuning with held-out validation --- for architectures making claims about \emph{regimes}; and a revision of our own applicability framework, including a sixth condition under which the ordering reverses. We also report, in the body rather than a footnote, three of our own claims that stricter protocols forced us to withdraw.
...
The estimator leads by $+0.860$ at $\phi{=}0$. We decompose that gap with a $2{\times}2$ over signal quality (constant vs.\ oracle $g$) and policy parameterization (hand-chosen vs.\ re-tuned coefficients; Appendix~\ref{app:coefsearch}). The two manipulations interact, so we report all four components, summing exactly (Appendix~\ref{app:decomp}): signal $+0.063$ ($7.4\%$), policy $+0.238$ ($27.7\%$), interaction $-0.047$ ($-5.5\%$), residual $+0.606$ ($70.5\%$). Repeated on time-in-band --- the metric that operationalizes the stated goal --- the shares are $8.7$, $29.4$, $-9.7$ and $71.7\%$, so the conclusion is not an artefact of measuring terminal ability instead of the target band. Against the model-free estimator of Section~\ref{sec:objmodel}, arguably the fairer comparator, the gap is $+0.580$ and the shares become $10.9$, $41.0$, $-8.1$ and $56.0\%$: the ordering is unchanged, only the magnitudes move.

\subsection{Matched tuning: signal quality does order performance, weakly}
\label{sec:matched}

The shared coefficients above were optimized against stable signals, so applying them to a noisy learned signal is not a fair test --- a point we owe to review. We therefore repeated the search per signal. A first version used one search each and validated on the profiles it had tuned on; it produced a non-monotonic ordering in which a learned $g$ beat a perfect one, which we initially reported as the headline. It does not survive a stricter protocol: \textbf{five independent searches per signal} (identical 250-candidate budget), validated on \textbf{50 profiles never used for tuning}. The held-out means are $+0.472$ (sd $0.051$) for a constant $g$, $+0.480$ (sd $0.068$) for a learned one, and $+0.519$ (sd $0.052$) for the oracle; a single larger run at 1200 candidates gives $+0.411$, $+0.545$ and $+0.656$ (Appendix~\ref{app:matchedtab}). For reference the estimator scores $+1.132$, and $+0.851$ without its objective model.


Three things follow. \textbf{Signal quality does order performance}: the oracle beats a constant in five of five searches, and the gap widens at a larger budget --- sparse search was masking the effect, and correcting for it strengthens signal quality rather than eliminating it. \textbf{Learning $g$ buys essentially nothing over a well-chosen constant} ($+0.480$ vs.\ $+0.472$), which bounds how much effort the SSP repays. And the effect is small in the relevant units: roughly $0.05$ against a remaining distance of $0.4$--$0.6$. Knowing what you need helps, and is nowhere near enough. The 1200-candidate column is a single exploratory run; the drop in its constant-$g$ cell is the tuning-set overfitting we warn about elsewhere.
...
The argmax contrast is not a clean isolation of sampling noise: changing the action rule moves the entire closed loop --- visitation, fatigue, rest frequency, and the resulting $g$ estimates. $\Gamma$\_argmax attains higher signal validity ($+0.604$ vs.\ $+0.471$) under an identical learning rule, because systematic visitation yields cleaner estimates. Read P4 as ``the deterministic policy is better,'' not as ``sampling noise costs this much.''

\textbf{P6 confirmed but policy-dependent.} Validity for $\Gamma$\_learned falls from $+0.471$ to $+0.113$; for $\Gamma$\_argmax it remains $+0.448$ at $\phi{=}1.2$. Learnability is therefore not bounded by non-stationarity as such --- it is bounded by non-stationarity \emph{under a stochastic policy}.
...
Condition (4) fails outright, and we can price it. An outside observer of the outcome stream alone reconstructs the agent's effective ability to within $0.32$, on a scale where levels sit $1.0$ apart. Nothing is private.
...
What would we expect if that is right? Replace the terminal objective with one that \emph{cannot} be banked --- mean readiness, the score you would get if the exam were today, averaged over every session --- and the ordering should reverse. It does, under a conjunction of three. At our own setting ($\phi{=}0.6$, 40 sessions) $\Gamma$ still loses ($d_z{=}{-}1.07$). Stretch the horizon to 400 and it passes the model-based estimator ($+0.076$, $d_z{=}{+}2.09$). Add severe adversity and it beats \emph{both} ($+0.044$ and $+0.031$, $d_z{=}{+}0.73$ and $+0.61$) while dropping out more often --- so among survivors the margin is wider still (Appendix~\ref{app:maintenance}).
...
Can a homeostatic agent learn its satiation signal from observable consequences and use it to regulate toward an external goal? Analyzed as the paired design it is: signal quality matters, and matters least. A perfect signal is worth $7.4\%$ of the gap to a simple estimator, six policy coefficients $27.7\%$, and the rest is neither. Under matched tuning the ordering does track signal quality --- by about $0.05$, while a \emph{learned} signal is indistinguishable from a constant and the estimator stays ahead even stripped of its objective model.
