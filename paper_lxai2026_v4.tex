...
Sweeping raises its own problem: at extreme adversity an environment can stop discriminating between good and bad policies. We therefore re-run a quality gate at \emph{every} point --- hiding at level 1 must stay unprofitable, the quitting boundary must bite, an oracle policy must beat random. It passes for $\phi\le1.2$ and fails at $\phi\ge1.6$, where severe fatigue makes hiding viable and the trap dissolves. Because that exclusion is a judgement call, we report the crossover contrast with the excluded point included (Appendix~\ref{app:gate}); other sweep-dependent claims are not re-derived there.
...
Each arm isolates one suspect. \texttt{$\Gamma$\_oracle}, \texttt{$\Gamma$\_fixed} and \texttt{$\Gamma$\_naive} vary only the satiation signal; \texttt{$\Gamma$\_argmax} and \texttt{$\Gamma$\_restdrive} vary only the policy; \texttt{rule} and \texttt{estimator} are non-homeostatic baselines, each run with and without its rest mechanism so viability separates from level selection. The estimator is a ten-line agent that tracks ability from outcomes and picks the level its model says is best.
...
\begin{table}[H]
  \caption{Held-out validation gain by signal and search budget. \textbf{Key take-away:} how much signal quality is worth is not a property of the signal --- it triples when the policy is searched harder, and a learned $g$ goes from matching a constant to beating it.}
  \label{tab:matched}
  \centering\small
  \begin{tabular}{lrrr}
    \toprule
    ...
    \midrule
    constant $g$ & $+0.472$ (0.051) & $+0.452$ (0.036) & --- \\
    learned $g$  & $+0.480$ (0.068) & $+0.584$ (0.083) & --- \\
    oracle $g$   & $+0.519$ (0.052) & $+0.610$ (0.044) & $+0.047 \to +0.158$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
That equivalence does not survive a larger budget, and we report the reversal because it changes the reading. At 1200 candidates the oracle beats the constant in $3/3$ seeds and so does the learned signal, by $+0.158$ and $+0.132$ respectively. Searching the policy harder did not make the signal irrelevant; it made the signal \emph{matter more}. Correspondingly the interaction term of Section~6.2 flips from $-2.1\%$ to roughly $+11\%$.

We take the honest conclusion to be a claim about complementarity rather than about ranking. \textbf{A satiation signal is worth little until the channel can carry it}, which is why the effect looks small when the policy is hand-chosen or coarsely searched, and grows as the policy improves. Two of our earlier claims fall with this: that a learned $g$ buys nothing over a constant (true only at the smaller budget) and that signal quality is simply the smallest term (true under a fixed policy, misleading as a general statement). What survives, and what the title asserts, is that knowing what you need is necessary and far from sufficient --- roughly $59\%$ of the gap outlives both interventions even at the larger budget.
...
\textbf{P3 confirmed against the model-based estimator} --- though not against the stronger one of Section~\ref{sec:objmodel} --- with three qualifications. At $\phi{=}1.2$ the regulated agent overtakes by $+0.045$, CI $[+0.028,+0.062]$; since P3 is an existence claim, a max-statistic sign-flip permutation over all $\Gamma$ arms and valid $\phi$ gives $p{=}.0005$ (max $t{=}{+}5.18$ at that cell), so it survives multiplicity correction. But \textbf{the crossover belongs to the \texttt{$\Gamma$\_argmax} ablation, not the main agent}: \texttt{$\Gamma$\_learned} scores $+0.143$ against $+0.156$ and does not overtake. And $+0.045$ sits below our own $\delta{=}0.05$ threshold --- a robust reversal of ordering, not a large one. Excluding $\phi{=}1.6$ was conservative: including it the contrast grows to $+0.089$, $d_z{=}{+}1.54$.
...
That is the crux. Conditions (1), (2), (3) and (5) say regulation is \emph{needed}; only (4) says the agent must be the one doing it. It is load-bearing, and we treated all five as equals. Tellingly, $\Gamma$ gains ground exactly where (4) begins to hold: as adversity drives reconstruction error from $0.32$ to $0.86$, the estimator collapses and $\Gamma$ does not.
...
Three of our own claims fell here, each to one methodological choice. An unpaired test on a paired design hid a real signal effect. A single tuning search invented a non-monotonic ordering that five searches erased. An unablated baseline let us credit homeostasis for what a rest rule was doing. Regulatory architectures make claims about \emph{regimes} and are evaluated within-subject: dose--response sweeps, paired inference, multi-seed tuning with held-out validation, and ablation of the \emph{baselines} --- not only the proposal --- should be default.
...
\textbf{Signal validity is incompletely operationalized}: only $g_{\text{prog}}$, only against terminal expected gain, only as a five-point correlation, with $g_{\text{conf}}$ unvalidated --- so ``learning $g$'' means learning $g_{\text{prog}}$. Relatedly, $4a(1-a)$ peaks at $a{=}0.5$ while expected gain is maximized where accuracy is $0.354$, so fidelity is capped by functional form and the oracle arm feeds the policy a differently shaped signal than the policy was tuned for.
...
\label{app:decomp}
\begin{table}[H]
  \caption{Components of the $\phi{=}0$ gap on learning gain and time-in-band, against both baselines. \textbf{Key take-away:} the ordering of the terms is identical on both metrics and against both comparators.}
  \centering\small
  \begin{tabular}{lrrrr}
    \toprule
    Component & gain & \% & time-in-band & \% \\
    \midrule
    Signal quality (oracle $-$ constant, hand policy) & +0.063 & 7.4 & +0.050 & 8.7 \\
    Policy coefficients (re-tuned $-$ hand, constant $g$) & +0.238 & 27.7 & +0.169 & 29.4 \\
    Interaction & $-0.047$ & $-5.5$ & $-0.056$ & $-9.7$ \\
    Residual (estimator $-$ best $\Gamma$) & +0.606 & 70.5 & +0.414 & 71.7 \\
    \midrule
    Total gap vs.\ model-based estimator & +0.860 & 100.0 & +0.577 & 100.0 \\
    \midrule
    \multicolumn{5}{l}{\emph{against the model-free estimator (total $+0.580$): 10.9\%, 41.0\%, $-8.1$\%, 56.0\%}} \\
    \bottomrule
  \end{tabular}
\end{table}
