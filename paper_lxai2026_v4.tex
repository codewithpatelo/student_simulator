...
A $2{\times}2$ over signal quality (constant vs.\ oracle $g$) and policy parameterization (hand-chosen vs.\ searched coefficients) decomposes the gap exactly, interaction included (Table~\ref{tab:decomp}). Repeated on time-in-band the shares are $8.7$, $29.4$, $-9.7$ and $71.7\%$ --- though that metric is conditional on study sessions, and the $\Gamma$ arms rest far more than the baselines, so it measures level selection when studying rather than regulation over the full budget (Appendix~\ref{app:decomp}). The residual is what \emph{the two interventions we ran} leave unexplained. Some of it is the fixed action representation rather than anything structural: the logits read only the two drive deviations, discarding the per-level accuracy the agent already tracks, and feeding that back in buys a further $+0.196$ ($23.1\%$ of the gap) with no privileged information (Appendix~\ref{app:levelaware}). Counting it, the channel accounts for roughly half the gap against $7.7\%$ for the signal. The interaction is negative, which is worth pausing on: a searched policy extracts \emph{less} extra benefit from a perfect signal ($+0.047$) than the hand-chosen one does ($+0.065$). Either the random search failed to co-adapt the policy to the better signal, or six linear coefficients cannot express a control law that exploits it --- both readings point at channel capacity rather than at the signal.
...
\section{Is the gap a lack of information, or a channel that discards it?}
\label{app:levelaware}

A natural objection to our headline is that $\Gamma$ and the estimator are not representationally matched: the estimator tracks latent ability and applies a non-linear argmax over levels, while $\Gamma$'s logits read two drive deviations. On that reading the gap would be missing \emph{information}, not a narrow channel.

The agent's state settles it. $\Gamma$ already maintains a per-level accuracy average $\text{acc}[k]$, the current level, and per-level $g$ --- but \texttt{select()} builds its logits from $d_p$, $d_c$ and strain alone, discarding the rest. We therefore fed one quantity the agent already holds back into the channel, adding $\pm\gamma(\text{acc}[k]-0.5)$ to the \textsc{up} and \textsc{down} logits: nothing privileged, nothing the estimator can see that $\Gamma$ cannot.

\begin{table}[H]
  \caption{Effect of letting the policy read information the agent already tracks (100 paired profiles, $\gamma{=}6$). \textbf{Key take-away:} the information was never missing --- the channel was throwing it away.}
  \centering\small
  \begin{tabular}{lrrrrr}
    \toprule
    $\phi$ & $\Gamma$\_learned & level-aware & difference & $d_z$ & estimator \\
    \midrule
    0.0 & +0.248 & +0.448 & $+0.200$ & $+4.53$ & +1.132 \\
    0.6 & +0.138 & +0.214 & $+0.076$ & $+2.44$ & +0.360 \\
    1.2 & +0.143 & \textbf{+0.174} & $+0.031$ & $+1.45$ & +0.156 \\
    \bottomrule
  \end{tabular}
\end{table}

All contrasts are paired with $p<10^{-25}$. At $\phi{=}1.2$ the level-aware agent overtakes the model-based estimator without needing the deterministic ablation, so the crossover of Section~\ref{sec:policy} no longer rests on \texttt{$\Gamma$\_argmax} alone. On the 50 held-out profiles the effect is $+0.196$, or $23.1\%$ of the $\phi{=}0$ gap.

Two honest qualifications. The input gain $\gamma$ was chosen by hand and the effect is \emph{monotone} in it --- $+0.306$ at $\gamma{=}1$, $+0.435$ at $6$, $+0.651$ at $20$, saturating near $+0.706$ at $80$ --- so the figure we report is a conservative lower bound rather than a tuned optimum. And at high $\gamma$ the policy approaches ``go up when the level is easy, down when it is hard,'' which resembles the \texttt{rule} baseline; it does not collapse into it, scoring $+0.651$ against the rule's $+0.574$, but the resemblance is worth naming. Under adversity the widened channel still trails the model-free estimator ($+0.191$ vs.\ $+0.267$ at $\phi{=}1.2$, $\gamma{=}12$): the channel explains a great deal, not everything.

\section{Can the satiation mapping itself be learned?}
\label{app:mapping}

Our learning rule estimates $\text{acc}[k]$ from experience and then applies a fixed transfer function $4a(1-a)$. A reader may fairly object that this learns the \emph{argument} of an authored mapping rather than the mapping, and our own aligned-rule result shows the form is doing decisive work. So we asked whether the mapping can be learned instead.

The agent cannot observe learning gain. But it can observe whether its accuracy at a level is \emph{improving}, which is a proxy for how much that level is teaching it. We therefore set $g_{\text{prog}}[k]$ from the observed per-visit improvement in $\text{acc}[k]$, with no authored shape and no privileged constant such as the true optimum $a^\star$.

\begin{table}[H]
  \caption{Held-out gain at $\phi{=}0$ by how $g_{\text{prog}}$ is obtained. \textbf{Key take-away:} a mapping learned from an observable proxy beats the mapping we would have written, and recovers over half of what a ground-truth signal buys.}
  \centering\small
  \begin{tabular}{lrl}
    \toprule
    How $g_{\text{prog}}$ is obtained & gain & vs.\ constant \\
    \midrule
    Constant & +0.261 & --- \\
    Authored rule $4a(1-a)$ & +0.239 & $-0.022$ \\
    \textbf{Learned from observed improvement} & \textbf{+0.298} & $+0.037$ ($p{=}4\times10^{-19}$) \\
    Expected-gain oracle & +0.326 & $+0.065$ \\
    \bottomrule
  \end{tabular}
\end{table}

The learned mapping captures roughly $57\%$ of the oracle's advantage while using only what the agent can see. Combined with the widened channel of Appendix~\ref{app:levelaware} it reaches $+0.640$ against the estimator's $+1.107$ --- closing nearly half the gap with two changes that require no privileged information. The advantage is confined to the benign regime, as every signal effect in this paper is ($p{=}.66$ at $\phi{=}0.6$), and the improvement estimate is itself noisy at roughly six visits per level. What it establishes is narrower than ``the satiation mapping is learnable in general'' and stronger than the concession it replaces: in this environment the mapping can be induced from consequences, and doing so outperforms authoring it.
...
\subsection{Analysis code: paired inference and equivalence}
\label{app:paired}
