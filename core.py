...
def run_episode(agent,profile,seed,is_gamma,env_kw,log=False,horizon=None):
    ...
    m={"ganancia":round(env.h-profile["h0"],3),
       ...
       "tib_full":round(zpd/T,3),
       "survived":(not agent.abandono),
       "sessions_survived":(s+1),
       "gain_per_session":round((env.h-profile["h0"])/max(1,s+1),5),
